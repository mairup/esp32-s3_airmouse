...
create-breathe-table -> ByteArray:
  table := ByteArray 200
  200.repeat: |ticks|
    x := ticks / 200.0
    breathe-factor := 0.0
...
class RgbIndicator:
  led /RgbLed
  ble /BleServer
  breathe-table_ /ByteArray ::= create-breathe-table
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        // ⚡ Bolt: Using pre-calculated lookup table instead of live math for performance
