...
// Precomputed breathing LED lookup table
// This reduces the per-tick calculation to an O(1) lookup, improving performance significantly.
BREATHE-TABLE ::= List 200: |ticks|
  x := ticks / 200.0
  breathe-factor := 0.0
  if x < 0.60:
    t := x / 0.60
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        // Optimization: Precomputed values save continuous math operations
        r = 0
        g = 0
        b = BREATHE-TABLE[flash-ticks % 200]
        flash-ticks++
