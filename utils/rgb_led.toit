...
class RgbIndicator:
  led /RgbLed
  ble /BleServer
  breathe-curve_ /List ::= List 200

  constructor .led .ble:
    // Precompute the breathing curve animation to avoid heavy float math in the 100Hz loop
    200.repeat: |ticks|
      x := ticks / 200.0
      breathe-factor := 0.0
      if x < 0.60:
      ...
      // Clamp the calculated value to 0-255 just in case the math exceeds bounds
      val := (255.0 * breathe-factor).to-int
      breathe-curve_[ticks] = val.clamp 0 255
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        // Playful Flutter-style spring-breathing cycle using precomputed O(1) LUT
        ticks := flash-ticks % 200
        r = 0
        g = 0
        b = breathe-curve_[ticks]
        flash-ticks++
