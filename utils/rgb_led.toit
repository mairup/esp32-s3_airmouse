...
// Precomputed breathing animation table to avoid floating point math in the run loop
BREATHE-TABLE ::= List 200: |i|
  x := i / 200.0
  factor := 0.0
  if x < 0.60:
    t := x / 0.60
    factor = (math.pow 2.0 (-8.0 * t)) * (math.sin (t * 22.0)) + 1.0
  else:
    t := (x - 0.60) / 0.40
    factor = 0.5 * (math.cos (t * math.PI) + 1.0)
  (255.0 * factor).to-int
