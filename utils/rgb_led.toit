...
// Optimization: Pre-calculated Look-Up Table (LUT) for the breathing animation.
// Avoids heavy floating-point and transcendental math (pow, sin, cos) in a 100Hz loop
// on the microcontroller, saving active CPU time and power.
//
// Generated from:
// // Playful Flutter-style spring-breathing cycle (period = 2.0s -> 200 steps at 100Hz)
// x := ticks / 200.0
// if x < 0.60: // Swell & Spring: Flutter Elastic-Out spring bounce at the peak
//   t := x / 0.60
//   breathe-factor = (math.pow 2.0 (-8.0 * t)) * (math.sin (t * 22.0)) + 1.0
// else:        // Cosine decay: Extremely soft, soothing fade-out
//   t := (x - 0.60) / 0.40
//   breathe-factor = 0.5 * (math.cos (t * math.PI) + 1.0)
// b = (255.0 * breathe-factor).to-int
BREATHE-CURVE-LUT ::= [
  255, 299, 338, 371, 396, 415, 427, 431, 430, 422,
  410, 393, 373, 351, 327, 303, 280, 257, 237, 219,
  204, 192, 183, 177, 174, 175, 178, 183, 191, 200,
  210, 220, 231, 242, 252, 261, 270, 277, 282, 287,
  289, 291, 291, 289, 287, 284, 280, 276, 271, 266,
  261, 256, 252, 248, 245, 242, 240, 239, 238, 238,
  239, 240, 241, 243, 245, 247, 249, 251, 253, 255,
  257, 259, 260, 261, 262, 262, 262, 262, 261, 261,
  260, 259, 258, 257, 256, 255, 254, 253, 253, 252,
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        r = 0
        g = 0
        b = BREATHE-CURVE-LUT[flash-ticks % 200]
        flash-ticks++
