...
BREATHE-TABLE_ /ByteArray ::= create-breathe-table_

create-breathe-table_ -> ByteArray:
  table := ByteArray 200
  200.repeat: |ticks|
    x := ticks / 200.0
    breathe-factor := 0.0
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        // ⚡ Bolt: Precomputed lookup table to save CPU cycles from expensive math at 100Hz
