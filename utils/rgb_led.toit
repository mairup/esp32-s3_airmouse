...
// Precompute the breathing animation to avoid heavy float math in the run loop
compute-breathe-table_ -> ByteArray:
  table := ByteArray 200
  200.repeat: |ticks|
    x := ticks / 200.0
    breathe-factor := 0.0
  ...
  run_ -> none:
    ...
    while true:
      ...
      if state == BleServer.STATE-STOPPED:
        ...
        // Uses precomputed BREATHE-TABLE_ for O(1) lookup to save CPU cycles
